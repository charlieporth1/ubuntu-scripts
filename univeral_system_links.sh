...
sudo ln -s $PROG/{ctp-dns,nginx-dns-rfc,ctp-fail-over-dns}.service /etc/systemd/system/

if [[ "$HOSTNAME" == 'ctp-vpn' ]]; then
	sudo ln -s $PROG/{ctp-yt-ttl-dns,ctp-YouTube-Ad-Blocker,ctp-dns-dnscrypt}.service /etc/systemd/system/
fi
