...
echo $REGEX_EXCLUDE
parallel -j4 --lb echo "{}" :::: \
<(pihole --regex -l | grep --line-buffer "$REGEX_EXCLUDE" | awk '{print $2}') | parallel $parallel_args_pihole pihole --regex -d
...
pihole -b -l | grep --line-buffer "$RM_LIST" | awk '{print $2}' | parallel $parallel_args_pihole pihole -b -d
sleep 1.5s
echo "starting remove whitelist"
pihole -w -l | grep --line-buffer "$RM_LIST" | awk '{print $2}' | parallel $parallel_args_pihole  pihole -w -d
sleep 1.5s
###BLOCK-COMMENT
