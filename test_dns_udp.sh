...
DNS UDP_ARGS="+tries=$TRIES +dnssec +ttl +edns +timeout=$TIMEOUT -t $qtype -4 +retry=$TRIES +ttlunits +notcp"

dns_global=`dig $QUERY $isAuto @$HOST $DNS UDP_ARGS`
dns_master=`dig $QUERY $isAuto @master.$HOST $DNS UDP_ARGS`
dns_local=`dig $QUERY $isAuto @$server $DNS UDP_ARGS`
dns_external=`dig $QUERY $isAuto @$EXTENRAL_IP $DNS UDP_ARGS`

dns_local_test=`echo "$dns_local" | grepip --ipv4 -o | xargs`
dns_external_test=`echo "$dns_external" | grepip --ipv4 -o | xargs`

dns_logger "Running DNS UDP TEST"
...
if [[ -z "$isAuto" ]]; then
	...
	if [[ -z "$dns_local_test" ]]; then
		...
		if [[ "$server" == "$local_interface" ]] && [[ '-a' != "$1" ]]; then
			if [[ -f "$pihole_bin" ]] || [[ `systemctl-exists $service` ]]; then
				if [[ $(systemctl-inbetween-status $service) == 'false' ]]; then
					dns_logger "DNS UDP Failed restarting pihole_bin :$pihole_bin:"
		...
		dns_logger "DNS UDP: extenal failed posiable firewall issue"
		dns_logger "DNS UDP: extenal failed posiable firewall issue"
		...
		dns_logger "Test DNS UDP Success"
	fi
fi
