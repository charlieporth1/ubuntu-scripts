...
if [ "$(cat /mnt/HDD/buildstatusver.txt)" != "$(cat /mnt/HDD/buildstatusvernew.txt)" ]
then
...
if [ ! -z "$var" ]
then 
sendemail -f $USER@otih-oith.us.to -t $phonee  -m "Apple Successfully finished processing the build please submit the app Date: $(date) and Version: $(cat /mnt/HDD/buildstatusver.txt) " -s smtp.gmail.com:587 -o tls=yes -xu $usr  -xp  $passwd#fi
fi
