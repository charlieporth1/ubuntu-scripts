...
if [[ "$pre" != "$cmd" ]]; then
	...
	do
		...
		echo "var " $var
		if [[ ! -z "$var" ]]; then
			sendemail -f $USER@otih-oith.us.to -t $phonee  -m "Apple Successfully finished processing the build STUDENT please submit the app Date: $(date) and Version: $pre " -s smtp.gmail.com:587 -o tls=yes -xu $usr  -xp  $passwd
			killall ruby
			sleep 3s
			break
		else
...
if [[ "$preT" != "$cmdT" ]]; then
	...
	do
		...
		echo "varT " $varT
		if [[ ! -z "$varT" ]]; then 
			sendemail -f $USER@otih-oith.us.to -t $phonee  -m "Apple Successfully finished processing the build TEACHER please submit the app Date: $(date) and Version: $pre " -s smtp.gmail.com:587 -o tls=yes -xu $usr  -xp  $passwd
			killall ruby
			sleep 3s
			break
		else
