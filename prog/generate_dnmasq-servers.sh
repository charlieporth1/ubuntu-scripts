...
IP_REGEX="($IP_REGEX((#[0-9]{1,5})?))"

#OG_FILE=$DNSMASQ/10-servers.conf
DNS_LIST_FILE=$HOLE/custom-dns-server.conf
PIHOLE_FILE=$DNSMASQ/01-pihole.conf
OUT_FILE=$DNSMASQ/10-servers-generated.conf
PIHOLE_ACTIVE_DNS=`grep -oE "server=$IP_REGEX" $PIHOLE_FILE | grepip -o4 | ip-sort`
PIHOLE_ACTIVE_DNS_GREPIFY=`bash $PROG/grepify.sh $PIHOLE_ACTIVE_DNS`


MIN_QUERY_TIME_AMOUNT=`convert_time_seconds 25 m`
