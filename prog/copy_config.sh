...
# RMs
sudo rm -rf $INSTALL_CONFIG_D/sysctl.d/*.conf
...
if [[ -d $DEFAULT_PROG ]]; then
	for i in $CONF_PROG_DIR/*.{sh,service,bash,txt,list,regex,py}; do
  		if [ -r $i ]; then
       			ln -s $i $DEFAULT_PROG
    		fi
	done
...
sudo ln -s $CONF_PROG_DIR/route-dns/ $ROUTE/
if [ -d $ROUTE ]; then
  for i in $$CONF_PROG_DIR/route-dns/*.{sh,toml}; do
    if [ -r $i ]; then
      ln -s $i $ROUTE/
    fi
  done
