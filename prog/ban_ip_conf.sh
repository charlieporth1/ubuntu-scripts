...
function create_ip-set-allow() {
	...
	local LIST_TYPE="${2:-put-type-of-alowlist-here}"
        ...
        declare -gx IPSET_BK_NAME=$LIST_TYPE-alwlst-$LIST_NAME
        ...
        sudo iptables -I OUTPUT -m set --match-set $IPSET_BK_NAME src -j ACCEPT
        sudo iptables -I INPUT -m set --match-set $IPSET_BK_NAME src -j ACCEPT
        sudo iptables -I FORWARD -m set --match-set $IPSET_BK_NAME src -j ACCEPT
...
function create_ip-set() {
	...
	local LIST_TYPE="${2:-put-type-of-blacklist-here}"
        ...
        declare -gx IPSET_BK_NAME=$LIST_TYPE-blklst-$LIST_NAME
        declare -gx IPSET_FILE=$IPSET_BK_NAME.ipset
        declare -gx IPSET_FILE_FULL=$CONFIG_IP_SET_DIR/$IPSET_FILE
