...
echo "$(bash $PROG/dnsmasqify.sh ${DOMAINS_TO_TEST_WHITE[@]})" | parallel --lb echo 'server={}127.0.0.1#5053' | sudo tee -a $OUT_FILE
...
echo "$(bash $PROG/dnsmasqify.sh ${SMALL_DOMAINS_LIST[@]})" | parallel --lb echo 'server={}127.0.0.1#5053' | sudo tee -a $OUT_FILE
...
echo "$(bash $PROG/dnsmasqify.sh ${EXTRA_DOMAINS_LIST[@]})" | parallel --lb echo 'server={}127.0.0.1#5053' | sudo tee -a $OUT_FILE
