...
. /usr/bin/cred.sh
sendemail -f $USER@otih-oith.us.to -t $phonee  -m "Someone has logged into your one of your servers under the User: $USER; and the Server: $HOSTNAME; at the time of $(date); the external IP address of the login was from $llll; the GeoIP $(geoiplookup $llll | grep -i city | cut -d ':' -f 2 | cut -d ',' -f -7 ); the ISP of the login  $(geoiplookup $llll | grep -i ASNum | cut -d ':' -f 2  )"   -s smtp.gmail.com:587 -o tls=yes -xu $usr -xp  $passwd | GREP_COLOR='01;31' grep --color=always 'successfully!' | GREP_COLOR='01;36' grep  --color=always 'Email' | GREP_COLOR='01;33'  grep  --color=always 'sent'
exit 0
